...
st.set_page_config(
    page_title="Shravan Parthe — Developer & AI Engineer",
    page_icon="🚀",
    layout="wide",
...
.n1{{width:700px;height:700px;background:radial-gradient(circle,rgba(167,139,250,.18),transparent);top:-200px;left:-200px}}
...
.live-badge{{
  ...
  color:var(--green);background:rgba(52,211,153,.06);
}}
...
.ba-primary{{
  ...
  box-shadow:0 4px 24px rgba(94,243,255,.18);
}}
...
.hscene{{position:relative;display:flex;justify-content:center;align-items:center}}
.scene-box{{
...
.rkt-svg{{position:absolute;right:32px;top:40px;animation:rfloat 5s ease-in-out infinite;z-index:5}}
@keyframes rfloat{{0%,100%{{transform:translateY(0) rotate(20deg)}}50%{{transform:translateY(-14px) rotate(15deg)}}}}
.astro-svg{{position:absolute;top:50%;left:50%;transform:translate(-50%,10%);animation:afloat 6s ease-in-out infinite;z-index:5}}
@keyframes afloat{{0%,100%{{transform:translate(-50%,10%) rotate(-2deg)}}50%{{transform:translate(-50%,-2%) rotate(2deg)}}}}
#stats,#skills,#missions,#log,#contact{{padding:5rem 0}}
.qw{{border-top:1px solid var(--bd);padding:5rem 0;text-align:center;position:relative}}
.qt{{font-family:var(--B);font-style:italic;font-size:1rem;line-height:2.1;color:var(--muted);max-width:560px;margin:0 auto 1.2rem}}
.qt em{{color:var(--cyan);font-style:normal;font-weight:600}}
...
<body>
<canvas id="c"></canvas>
<div class="nb n1"></div><div class="nb n2"></div><div class="nb n3"></div><div class="nb n4"></div>
...
<nav id="nav">
  <div class="ni">
    ...
    <div class="nl">
      ...
      <div class="live-badge"><div class="ld"></div>Available</div>
    </div>
  </div>
</nav>
...
<section id="hero">
  <div class="w">
    <div class="hg">
      <div>
        <div class="coord">Shravan Parthe • Pimpri-Chinchwad • Maharashtra</div>
        <h1 class="ht"><span class="glow">Shravan</span><br>Parthe.</h1>
        <div class="hrole">Backend Engineer &amp; AI Developer</div>
        <p class="hdesc">
          <strong>3rd year B.Tech CSE (AI&amp;ML)</strong> student building production-grade systems in Java, Spring Boot, and Python.
          Currently at <strong>Innovexis</strong>, integrating real-world Gen AI into scalable backends.
        </p>
        <div class="hacts">
          <a href="mailto:shravanparthe@gmail.com" class="ba ba-primary">&#128640; Get In Touch</a>
          <a href="https://github.com/Shravan157" target="_blank" rel="noopener noreferrer" class="ba ba-ghost">&#9889; GitHub</a>
          <a href="{RESUME_URL}" target="_blank" rel="noopener noreferrer" download="Shravan_Parthe_Resume.pdf" class="ba ba-resume">&#128196; Resume</a>
        </div>
      </div>
...
<section id="skills"><div class="w"><h2>Tech Stack</h2></div></section>
<section id="projects"><div class="w"><h2>Featured Projects</h2></div></section>
<section id="experience"><div class="w"><h2>Professional Experience</h2></div></section>
<section id="contact"><div class="w"><h2>Get In Touch</h2></div></section>

<div class="w">
  <div class="qw">
    <p class="qt">Heavy is the head — <em>it's a blessing and a curse.</em></p>
  </div>
</div>
</div>
...
(function() {{
  ...
  var stars = [];
  for (var i=0;i<260;i++) stars.push({{x:Math.random()*innerWidth,y:Math.random()*innerHeight,r:Math.random()*1.5+.2,a:Math.random(),s:Math.random()*0.007+.002,p:Math.random()*Math.PI*2}});
  function tick() {{
    ctx.clearRect(0,0,c.width,c.height);
    for (var i=0;i<stars.length;i++) {{
      var s = stars[i]; s.p += s.s; s.a = .3 + .7*(Math.sin(s.p)*.5 + .5);
